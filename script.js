...
function sendMail(event) {
  ...
  if (params.name && params.email && params.phone && params.message) {
    emailjs.send("service_kd8w2sd", "template_ou5k3wk", params).then(
      () => {
...
function updatePageWithTranslations(data) {
  ...
  aboutSection.querySelector(".cta-button").textContent =
    data.about_us.contact_button;
  ...
  contactSection.querySelector(".contact-content p").textContent =
    data.contact.content;

  // Update placeholders in the contact form
  document.getElementById("name").placeholder =
    data.contact.form.name_placeholder;
  document.getElementById("email").placeholder =
    data.contact.form.email_placeholder;
  document.getElementById("phone").placeholder =
    data.contact.form.phone_placeholder;
  document.getElementById("message").placeholder =
    data.contact.form.message_placeholder;
  document.querySelector(".submitButton").textContent =
    data.contact.form.submit_button;
  ...
  const copyright = document.querySelector(
    'footer p[data-translation-key="footer.copyright"]'
  );
...
function updateActiveNavLink() {
  ...
  const homeLink = document.querySelector(
    'nav a[data-translation-key="menu.home"]'
  );
  const aboutSection = document.getElementById("about");
...
function handleNavLinks() {
  ...
  const navLinks = document.querySelectorAll(
    "nav a[data-translation-key], .cta-button"
  );
...
function generateServiceHTML(serviceData) {
  ...
  if (serviceData && serviceData.length > 0) {
    // Check if serviceData is defined and not empty
...
function handleReadMoreLinks() {
  ...
  document.querySelectorAll(".read-more").forEach((link) => {
    link.addEventListener("click", function (event) {
      ...
      const descriptionFull = service.querySelector(
